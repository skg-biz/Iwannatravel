...
export type TravelPersonality = 'healer' | 'adventurer' | 'romantic' | 'wanderer' | 'planner';
...
export const PRICE_RANGES: Record<PriceRange, { label: string; description: string; range: string }> = {
  budget: { label: '가성비 여행', description: '알뜰하게, 하지만 알차게', range: '~50만원' },
  mid: { label: '적당한 사치', description: '나를 위한 작은 사치', range: '50~150만원' },
  luxury: { label: '나를 위한 투자', description: '특별한 나를 위한 특별한 여행', range: '150만원~' },
};
